...
const CustomerLayoutInner = ({ children }) => {
  ...
  const { restaurant, customer, logout, cartCount, cartTotal, loading, error, loadRestaurant } = useCustomer();
  const navigate = useNavigate();

  const isHomePage = location.pathname === `/r/${slug}`;
      ...
      <header className="sticky top-0 z-50 safe-top bg-white border-b border-gray-100">
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3 min-w-0 flex-1">
            ...
            <Link to={`/r/${slug}`} className="flex items-center gap-2.5 min-w-0 hover:opacity-80 transition-opacity">
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            <button onClick={() => navigate(`/r/${slug}/cart`)} className="relative p-2.5 rounded-full hover:bg-gray-50 transition-colors">
            ...
            {customer ? (
              ...
              <button onClick={() => navigate(`/r/${slug}/login`)} className="p-2.5 rounded-full hover:bg-gray-50 transition-colors">
